$(document).ready(function() {
  ...
  var $referral_patient_consent_dialog = $('<div></div>').html("Please attach a copy of the patient consent!").dialog({
    autoOpen: false,
  ...
  if ($("#referral_person_full_name").length) {
    ...
    $("#referral_person_full_name").autocomplete({
      delay: 500
...
function token_input_existing_values(selector) {
  ...
  for (var i in existing_values) {
    items.push({
      id: existing_values[i],
      name: $(selector + " option[value='" + existing_values[i] + "']").text()
    });
  }
  return items;
}
...
function person_info(organisation, sap_number, dob) {
  info = "";
  if (organisation !== undefined) info += "<span><b>Organisation:</b> " + organisation + "</span> ";
  if (sap_number !== undefined) info += "<span><b>SAP Number:</b> " + sap_number + "</span> ";
  if (dob !== "") info += "<span><b>Dob:</b> " + dob + "</span> ";
  return info;
}
